...
from pyvo.mivot.class_wrappers.stc_classes.measures import Position, Time, GenericMeasure
from pyvo.mivot.class_wrappers.astropy_wrapper.sky_coord import SkyCoord
from pyvo.mivot.class_wrappers.component_builder import ComponentBuilder
...
class ModelViewer(object):
    ...
    def __init__(self, resource, parsed_votable=None, votable_path=None):
        '''
        Constructor
        ...
        elif parsed_votable is not None:
            self._get_mapping_block(parsed_votable)
    """
    Properties
    """
    @property
    ...
    def _get_mapping_block(self, parsed_votable):
        for resource in parsed_votable.resources:
            mapping_string = resource.model_mapping.mapping_block
            if mapping_string is not None:
                logger.info("Mapping block found")
                self._resource = resource
                self._resource_seeker = ResourceSeeker(self._resource)
                self._annotation_seeker = AnnotationSeeker(etree.fromstring(mapping_string))
                # Let's assume that there is is only one mapped table
                self.connect_table(None)
                return

        logger.error("No mapping block found")
